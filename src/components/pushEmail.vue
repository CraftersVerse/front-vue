<template>
    ...
    <div class="form-wrapper">
      <form @submit.prevent="pushEmail" class="form">
  ...
  export default {
      ...
      async pushEmail() {
        try {
          ...
          const response = await axios.post(
            'http://127.0.0.1:8000/api/application/push-email/',
            formData,
            {
              headers: {
          ...
          this.studentData = response.data['Email Data'];
          this.error = '';
          this.form.examVenue = '';
    ...
    toCSV() {
      ...
      this.downloadFile(csvData, 'csv', 'EmailSent.csv');
    },
  ...
  const headings = 'Id,Name,Email'; // Updated headings
...
toExcel() {
  ...
  this.downloadFile(excel, 'excel', 'EmailSent_Data.xlsx');
},
