  ...
  <div>
    <div class="container">
      <div class="form-wrapper">
        <form @submit.prevent="displayAttendance" class="form">
          <!-- Exam Venue Input -->
          <div class="form-group">
            <label for="examVenue" class="label">Enter Exam Venue</label>
            <div class="input-field">
              <input id="examVenue" v-model="form.examVenue" @input="validateExamVenue" type="text" class="form-control"
          ...
          <div class="form-group">
            ...
            <div class="input-field">
              <input id="subjectCode" v-model="form.subjectCode" @input="validateSubjectCode" class="form-control"
    ...
    <main class="main table" ref="student_table">
      ...
      <section class="table__body">
        <table>
          ...
          <tbody>
            <tr v-for="student in studentDataFiltered" :key="student.id">
              <td>{{ student.id }}</td>
              <td>{{ student.name }}</td>
...
export default {
  ...
  computed: {
    studentDataFiltered() {
      ...
      return this.studentData.filter((student) =>
        ...
        student.venue.toLowerCase().includes(search)
      );
    },
  },
  methods: {
    async displayAttendance() {
      try {
        const formData = {
        ...
        const response = await axios.post(
          'http://54.88.151.16:8000/api/application/display-attendance/',
          formData,
          {
            headers: {
        ...
        this.studentData = response.data['Attendance Student'];
        this.error = '';
        this.form.examVenue = '';
        //this.form.subjectCode = '';
        this.searchTable(); // Trigger search after updating data
        this.showTable();
        this.message = 'Student Attendance Data Generated Successfully';
    ...
    searchTable() {
      ...
      tableRows.forEach((row, i) => {
        ...
        const examVenue = row.querySelector('td:nth-child(3)').textContent.toLowerCase();
        const subjectCode = row.querySelector('td:nth-child(4)').textContent.toLowerCase();
        const subjectName = row.querySelector('td:nth-child(5)').textContent.toLowerCase();
        const venue = row.querySelector('td:nth-child(6)').textContent.toLowerCase();
    ...
    toCSV() {
      ...
      this.downloadFile(csvData, 'csv', 'Attendance_Data.csv');
    },
    convertToCSV() {
  const table = document.querySelector('table');
  const tbodyRows = table.querySelectorAll('tbody tr');

  const headings = 'Id,Name,ExamVenue,SubjectCode,SubjectName,Venue'; // Updated headings
  ...
  return csv;
    ...
    toExcel() {
      ...
      this.downloadFile(excel, 'excel', 'Attendance_Data.xlsx');
    },
...
.form-group {
  ...
  flex-direction: column;
  align-items: center;
  /* Aligns the entire input group to the center */
}
...
.label {
  ...
  /* Moves the label to the left within the input group */
  margin-left: 5px;
  /* Provides some spacing between label and input field */
