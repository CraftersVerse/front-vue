<template>
  <div>
    <!-- <form @submit.prevent="displayStudent" class="form">
    ...
    <main class="main table" ref="student_table">
      <section class="table__header">
        <h1>Enrollment Data Students</h1>
      ...
      <section class="table__body">
        <table>
          <thead>
            <tr>
              <th> Id </th>
              <th @click="sortTablebyName(true)"> Student <span class="icon-arrow">&UpArrow;</span> </th>
              <th> Subject Code</th>
              <th> Subject Name </th>
              <th> Venue </th>
              <th> Date </th>
...
export default {
  data() {
    return {
      ...
      searchValue: '', // Ensure searchValue is defined in data
    };
  },
  computed: {
    studentDataFiltered() {
      ...
      return this.studentData.filter((student) =>
        ...
        student.subjectCode.toLowerCase().includes(search) ||
        student.subjectName.toLowerCase().includes(search) ||
        student.examVenue.toLowerCase().includes(search) ||
        student.date.toLowerCase().includes(search) ||
        student.time.toLowerCase().includes(search)
      );
    },
  },
  methods: {
    ...
    async displayStudent() {
      ...
      try {
        const response = await axios.get('http://54.88.151.16:8000/api/application/display-student/');
        ...
        this.message = 'Enrollment Student Genereated Successfully';
    ...
    searchTable() {
      ...
      tableRows.forEach((row, i) => {
        ...
        const subjectCode = row.querySelector('td:nth-child(3)').textContent.toLowerCase();
        const subjectName = row.querySelector('td:nth-child(4)').textContent.toLowerCase();
        const examVenue = row.querySelector('td:nth-child(5)').textContent.toLowerCase();
        ...
        const isMatch =
          ...
          examVenue.includes(searchValue) ||
    ...
    toPDF() {
      ...
      const htmlContent = `<main class="table">${printableContent.innerHTML}</main>`;
    ...
    toCSV() {
      ...
      this.downloadFile(csvData, 'csv', 'Enrollment_data.csv');
    },
    convertToCSV() {
      const headings = 'Id,Name,Subject Code,Subject Name,Exam Venue,Date,Time';
      ...
      const tableData = [...tbodyRows]
        .map(row => {
          ...
          // Assuming the date column is at index 5 (adjust the index if needed)
          const dateCell = cells[5]; // Replace '5' with the appropriate index for the date column

          // Format the date before joining the cells' content
          const formattedDate = this.formatDate(dateCell.textContent.trim());

          // Replace the original date with the formatted date in the array of cell data
          cells[5].textContent = formattedDate; // Replace '5' with the appropriate index for the date column
    ...
    toExcel() {
      ...
      const table = document.querySelector('table');
      const tbodyRows = table.querySelectorAll('tbody tr');
      const headings = 'id\tstudent\tsubject code\tsubject name\texam venue\tdate\ttime'; // Updated headings
      ...
      const excel = headings + '\n' + tableData;
      this.downloadFile(excel, 'excel', 'Enrollment_Data.xlsx');
    },
...
thead th {
  ...
  width: calc(100% / 7);
  /* Distribute the width evenly among the seven columns */
  text-align: center;
...
.success-message {
  position: fixed;
  top: 20px;
  /* Adjust the vertical position above the form */
  left: 56%;
  ...
  border-radius: 5px;
  min-width: 200px;
  /* Adjust the minimum width of the message box */
  display: flex;
  justify-content: space-between;
  align-items: center;
}
...
.message-text {
  margin-right: 10px;
  /* Adjust margin between text and close button */
}
...
.btn-submit {
  ...
  color: #007bff;
  /* Change text color to a suitable color */
  border: 2px solid #007bff;
  /* Add a border with the same color as the text */
  border-radius: 5px;
