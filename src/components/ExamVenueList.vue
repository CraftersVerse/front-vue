<template>
    <div>
      <h1>Exam Venues</h1>
      <div v-if="confirmationMessage">{{ confirmationMessage }}</div>
      <div v-if="noDataMessage">{{ noDataMessage }}</div>
      <table v-if="examVenues.length > 0">
        <thead>
          <tr>
            <th>Venue ID</th>
            <th>Venue Number</th>
            <th>Capacity</th>
          </tr>
        </thead>
        <tbody>
            <tr v-for="venue in examVenues" :key="venue.id">
                <td>
                    <template v-if="editMode === venue.id">
                        <input v-model="editVenueData.venueID" type="text" />
                    </template>
                    <template v-else>
                        {{ venue.venueID }}
                    </template>
                </td>
                <td>
                    <template v-if="editMode === venue.id">
                        <input v-model="editVenueData.venueNum" type="text" />
                    </template>
                    <template v-else>
                        {{ venue.venueNum }}
                    </template>
                </td>
                <td>
                    <template v-if="editMode === venue.id">
                        <input v-model="editVenueData.capacity" type="number" />
                    </template>
                    <template v-else>
                        {{ venue.capacity }}
                    </template>
                </td>
                <td>
                    <template v-if="editMode === venue.id">
                        <button @click="saveEdit(venue)">Save</button>
                        <button @click="cancelEdit">Cancel</button>
                    </template>
                    <template v-else>
                        <button @click="editVenue(venue)">Edit</button>
                        <button @click="deleteVenue(venue.id)">Delete</button>
                    </template>
                </td>
            </tr>
        </tbody>
      </table>
    </div>
  </template>
  

<script>
    import { ref, onMounted } from 'vue';
    import { createClient } from '@supabase/supabase-js';

    const supabaseUrl="https://lggilvbvpcjmisfuzlfo.supabase.co"
    const supabaseKey="eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImxnZ2lsdmJ2cGNqbWlzZnV6bGZvIiwicm9sZSI6ImFub24iLCJpYXQiOjE2OTgyNTUyNjYsImV4cCI6MjAxMzgzMTI2Nn0.4L0CLvvH208iOfcn_8nYWuWWhZ3ZNfi2q3ru_V8pjko"
    const supabase = createClient(supabaseUrl, supabaseKey);

    export default {
        setup() {
            const examVenues = ref([]);
            const confirmationMessage = ref(null);
            const noDataMessage = ref(null);
            const editMode = ref('');
            const editVenueData = ref(null);
            

            const fetchExamVenues = async () => {
                try {
                    const { data, error } = await supabase
                    .from('application_examvenue')
                    .select('*');

                    if (error) {
                        console.error('Error fetching data from Supabase:', error.message);
                    } else {
                        if (data.length === 0) {
                            console.log('No data to show');
                            noDataMessage.value = 'No data to show';
                            // console.log(noDataMessage.value)
                            
                        } else {
                            examVenues.value = data;
                            noDataMessage.value = null;
                            
                        }
                    }
                } catch (error) {
                    console.error('Error fetching data from Supabase:', error.message);
                }
            };

            const editVenue = (venue) => {
                // Enter edit mode for the selected venue
                editMode.value = venue.id;
                // Create a copy of the original data for editing
                editVenueData.value = { ...venue };
            };

            const saveEdit = async (venue) => {
                try {
                    // Update the local state immediately for a responsive UI
                    examVenues.value = examVenues.value.map((v) => (v.id === venue.id ? { ...v, ...editVenueData.value } : v));

                    // Update the Supabase database with the edited data
                    const { data, error } = await supabase
                    .from('application_examvenue')
                    .update(editVenueData.value)
                    .eq('id', venue.id);

                    if (error) {
                    console.error('Error updating venue in Supabase:', error.message);
                    } else {
                    console.log('Venue updated successfully:', data);
                    // Set the confirmation message
                    confirmationMessage.value = 'Venue successfully edited!';
                    // Clear the confirmation message after a certain time (e.g., 3 seconds)
                    setTimeout(() => {
                        confirmationMessage.value = null;
                    }, 3000);
                    // Exit edit mode
                    editMode.value = null;
                    editVenueData.value = null;
                    }
                } catch (error) {
                    console.error('Error editing venue:', error);
                }
            };

            const cancelEdit = () => {
                // Exit edit mode without saving changes
                editMode.value = null;
                editVenueData.value = null;
            };

            const deleteVenue = async (venueId) => {
                try {
                    // Update the local state
                    examVenues.value = examVenues.value.filter((venue) => venue.id !== venueId);

                    // Update the Supabase database
                    const { data, error } = await supabase
                    .from('application_examvenue')
                    .delete()
                    .eq('id', venueId);

                    if (error) {
                    console.error('Error deleting venue from Supabase:', error.message);
                    } else {
                    console.log('Venue deleted successfully:', data);
                    }
                } catch (error) {
                    console.error('Error deleting venue:', error);
                }
            };

            // Function to handle inserts
            const handleInserts = (payload) => {
                console.log('Insert operation:', payload);
                // Perform actions to update your Vue state based on the insert
                // For example, fetch the updated data from the server
                fetchExamVenues();
            };

            // Listen to changes in DB
            supabase
                .channel('application_examvenue')
                .on('postgres_changes', { event: '*',schema: 'public', table: 'application_examvenue' }, handleInserts)
                .subscribe()

            onMounted(() => {
                fetchExamVenues(); // Call the method when the component is mounted
            });

            return { examVenues, fetchExamVenues, editVenue, deleteVenue, noDataMessage, saveEdit, cancelEdit, editMode, editVenueData };
        },
    };

</script>

<style scoped>

    * {
    margin: 0;
    padding: 0;

    box-sizing: border-box;
    font-family: sans-serif;
    }

    div{
        padding-left: 3%;
    }

    table {
        width: 78vw;
        height: auto;
        background-color: #fff5;

        backdrop-filter: blur(7px);
        box-shadow: 0 .4rem .8rem #0005;
        border-radius: .8rem;

        overflow: hidden;
    }

    thead th {
        position: sticky;
        top: 0;
        left: 0;
        background-color: #d5d1defe;
        cursor: pointer;
        text-transform: capitalize;
        /* width: calc(100% / 6); */
        width: auto;
        /* Distribute the width evenly among the six columns */
        text-align: center;
        /* Center align the text */
        padding: 0.5rem;
        /* Add some padding for better spacing */
    }

    th,td{
        border-collapse: collapse;
        padding: 1rem;
        text-align: center;
    }

    tbody tr:nth-child(even) {
        background-color: #0000000b;
    }

    tbody tr:hover {
        background-color: #0000001a;
    }

    button{
        margin-right: 10px;
    }
</style>